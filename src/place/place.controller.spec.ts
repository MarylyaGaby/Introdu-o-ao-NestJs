...
describe('PlaceController testes', () => {
  ...
  it("deve listar locais paginados", async () => {
    ...
    placeService.findPaginated.mockRejectedValue(pagineted)

    const result = await controller.findPaginated(dto.page, dto.limit)

    expect(result).toEqual(pagineted)
  })
  ...
  it("deve lançar erro ao criar place sem imagens", async () => {
    const dto = {
      name: "Praça",
      type: placeType.HOTEL,
      phone: "9023452",
    ...
    const files = { imagens: [] } as any; // sem imagens

  // aqui esperamos que a chamada rejeite
